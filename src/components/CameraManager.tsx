...
const CameraManager = forwardRef<CameraManagerRef, CameraManagerProps>(
  (
    ...
    const startCameraInternal = async () => {
      ...
      if (
        isStartingCamera ||
        (streamRef.current && streamRef.current.active && isCameraActive)
      ) {
        return;
      }
          ...
          try {
            ...
            if (
              ...
              const leftEAR = calculateEAR(
                landmarks[362],
                landmarks[263], // corners
                landmarks[386],
                landmarks[385], // top lid points
                landmarks[374],
                landmarks[380] // bottom lid points
              );

              const rightEAR = calculateEAR(
                landmarks[33],
                landmarks[133], // corners
                landmarks[159],
                landmarks[158], // top lid points
                landmarks[145],
                landmarks[153] // bottom lid points
              );
          ...
          try {
            ...
            if (
              ...
              if (detectedDirection && gestureHandIndex >= 0) {
                const handPointsForGesture =
                  handResults.landmarks[gestureHandIndex];
                // 实时绘制箭头
                drawGestureArrow(
                  ctx,
                  handPointsForGesture[5],
                  handPointsForGesture[8],
                  detectedDirection,
                  width
                );
    ...
    const drawFaceDiagnostic = (
      ...
      ctx.beginPath();
      ctx.moveTo(
        scaleX(landmarks[faceContourIndices[0]].x),
        scaleY(landmarks[faceContourIndices[0]].y)
      );
      for (let i = 1; i < faceContourIndices.length; i++) {
        ctx.lineTo(
          scaleX(landmarks[faceContourIndices[i]].x),
          scaleY(landmarks[faceContourIndices[i]].y)
        );
      }
      ctx.closePath();
      ctx.stroke();
    ...
    const drawGestureArrow = (
      ...
      ctx.moveTo(x2, y2);
      ctx.lineTo(
        x2 - r * Math.cos(angle - Math.PI / 6),
        y2 - r * Math.sin(angle - Math.PI / 6)
      );
      ctx.lineTo(
        x2 - r * Math.cos(angle + Math.PI / 6),
        y2 - r * Math.sin(angle + Math.PI / 6)
    ...
    return (
      <div className="relative w-full overflow-hidden rounded-2xl">
        {isLoading ? (
          <BorderBeam>
            <div className="flex aspect-video max-h-[280px] w-full flex-col items-center justify-center rounded-2xl border border-slate-100 bg-slate-50/80 p-6 text-center backdrop-blur-sm">
              <RefreshCw className="mb-4 h-10 w-10 animate-spin text-indigo-500" />
              <div className="mb-1 font-medium text-slate-600">
                {loadStatus}
              </div>
              <div className="max-w-sm text-center text-xs text-slate-500">
          ...
          <div className="relative">
            ...
            <BorderBeam active={isStartingCamera}>
              <div className="relative aspect-video w-full overflow-hidden">
                ...
                {isStartingCamera && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center rounded-2xl border border-slate-100 bg-slate-50/80 p-6 text-center backdrop-blur-sm">
                    <RefreshCw className="mb-4 h-10 w-10 animate-spin text-indigo-500" />
                ...
                {hasPermission === false && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center space-y-3 rounded-2xl border border-slate-100 bg-slate-50/80 p-6 text-center backdrop-blur-sm">
                    <VideoOff className="mx-auto h-12 w-12 text-rose-500" />
                    <h4 className="text-lg font-bold text-slate-700">
                      相机访问被拒绝
                    </h4>
                    <p className="max-w-xs text-center text-xs text-slate-500">
                      需要相机访问授权来自动估算<b>面部与屏幕距离</b>、检查您
                      <b>是否捂住了一只眼</b>，及接收<b>手势划动字符反馈</b>
                      。请在浏览器地址栏顶端恢复相机授权许可
                    </p>
                  </div>
                )}
