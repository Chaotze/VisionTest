...
export default function CreditCardCalibrator({
  ...
  return (
    <div className="w-full max-w-xl bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-2xl p-4 md:p-6 overflow-hidden relative">
      ...
      {/* Calibration Box */}
      <div className="w-full min-h-[300px] bg-slate-50 dark:bg-slate-950 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 p-4 flex flex-col items-center justify-center relative mb-6">
        ...
        <div
          ...
          className="bg-gradient-to-br mt-3.5 mb-6 from-indigo-600 via-indigo-700 to-violet-800 rounded-xl relative shadow-lg flex flex-col justify-between p-4 transition-all duration-75 select-none"
        >
          {/* Card Chip & Type icon */}
      ...
      <div className="flex gap-4">
        {onCancel && (
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2 border border-slate-200/50 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-semibold rounded-2xl transition"
          >
            取消
          </button>
        )}
